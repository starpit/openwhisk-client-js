...
class Client {
  ...
  noRetry (statusCode) {
    return !statusCode || statusCode === 400 || statusCode === 404 || statusCode === 409 || statusCode === 410 || statusCode === 500
  }
  ...
  request (method, path, options) {
    const req = this.params(method, path, options)
	  ...
	  const retryCount = options.retryCount || 10
		...
		console.error('needsRetry?=' + needsRetry + ' ETIMEOUT?=' + !!is_ETIMEOUT + ' isNormal=' + !!isNormalError + ' retryIter=' + iter)
