...
class Client {
  ...
  noRetry (statusCode) {
    return !statusCode || statusCode === 400 || statusCode === 404 || statusCode === 409 || statusCode === 410
  }
  ...
  request (method, path, options) {
    const req = this.params(method, path, options)
      ...
      if (options && options.timeout) {
		...
		const is_ETIMEOUT = err && (err.code === 'ETIMEDOUT' || err.message && err.message.code === 'ETIMEDOUT')
		...
		console.error('needsRetry?=' + !!needsRetry + ' ETIMEOUT?=' + !!is_ETIMEOUT + ' isNormal=' + !!isNormalError + ' retryCount=' + iter)
		console.error(err.statusCode)
		console.error(JSON.stringify(err.message))
		console.error(err.cause)
                if (needsRetry && (iter < (req.retryCount || 10))) {
                    retry()
                } else {
